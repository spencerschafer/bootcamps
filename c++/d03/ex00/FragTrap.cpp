...
void FragTrap::meleeAttack(std::string const &target) {
    srand(time(NULL));
    double random = (rand() % 1000) * 0.001;
    auto damage = (int) (this->meleeAttackDamage * random);
    std::cout << "LEFT RIGHT LEFT HIYAH! - [FR4G-TP " << this->name << " attacks " << target << " at arms length, causing " << damage
              << " points of damage]\n";
}
...
void FragTrap::grenadeAttack(std::string const &target) {
    srand(time(NULL));
    double random = (rand() % 1000) * 0.001;
    auto damage = (int) (this->rangedAttackDamage * random);
